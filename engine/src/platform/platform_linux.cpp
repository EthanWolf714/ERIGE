...
typedef struct internal_state {
    Display *display;
    xcb_connection_t *connection; //server connectiong
    xcb_window_t window; //ref to window 
    xcb_screen_t *screen; //ref to screen
...
FAPI b8 platform_startup(
    ...
    i32 height){
        ...
        //turn off key repeated
        //this call is global across the OS, 
        //key repeats won't work until reboot
        XAutoRepeatOff(state->display);

        
        //can't get display from connection
        //this is why this is done this way, display to get connection
        ...
        //use iterator to loop through screens
        //have to make this call or else we have no screen
        ...
        //after looping assign it 
        state->screen = it.data;

        //allocate a XID for window to be created
        state ->window = xcb_generate_id(state->connection);

        //need to register what types of events you need
        //XCB_CW_BACK_PIXEL = filling then window bg with single colour
        //XCB_CW_EVENT_MASK is required
        u32 event_mask = XCB_CW_BACK_PIXEL | XCB_CW_EVENT_MASK;

        //listen for keyboard and mous buttons
        ...
        //tell server to notify when wintdow manager
        //attempts to destroy window
...
void platform_console_write(const char* message, u8 colour){
    //FATAL, WARN, ERROR, INFO, DEBUG, TRACE
